...
import {actionPose,addRoom3D} from './sector3dKit'
...
const PALETTE={scrap:{metal:0x68737c,panel:0x28313a,accent:0xd77b45},patched:{metal:0x6d7b88,panel:0x203244,accent:0x3ccbe8},core:{metal:0x74869a,panel:0x192d43,accent:0x38bde8},advanced:{metal:0x707891,panel:0x252742,accent:0x7775ff},elite:{metal:0x80738f,panel:0x32223f,accent:0xac72ff},mythic:{metal:0x887d66,panel:0x39301d,accent:0xe8c75b}}
...
export default function SectorAvatar3D({pet,previewItem,compact=false,action='',onReady,onFallback}){
 ...
 useEffect(()=>{
  ...
  import('three').then(THREE=>{
   ...
   const el=host.current,scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(30,1,.1,100);camera.position.set(0,.05,8.35)
   ...
   renderer.setPixelRatio(Math.min(devicePixelRatio||1,low?1:1.55));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.32
   ...
   const metal=mat(cfg.metal,.76,.34),dark=mat(0x151b24,.82,.28),panel=mat(cfg.panel,.68,.34),glow=mat(cfg.accent,.32,.2,cfg.accent),glass=new THREE.MeshPhysicalMaterial({color:0x07121d,metalness:.3,roughness:.12,transmission:low?0:.08,clearcoat:low?.25:.8})
   ...
   // Equipment previews remain in the matching 2D artwork for now. The old
   // procedural gear used unrelated coordinate systems and visually exploded
   // around the chassis on mobile.
   add(new THREE.CylinderGeometry(1.85,2.1,.16,low?28:48),mat(0x151d29,.45,.42),scene,[0,-2.28,0]);const floorRing=add(new THREE.TorusGeometry(1.58,.025,10,low?36:64),glow,scene,[0,-2.18,0],[1,.45,1]);floorRing.rotation.x=Math.PI/2
   scene.add(new THREE.HemisphereLight(0xc9e3f2,0x111722,2.35));const key=new THREE.DirectionalLight(0xffffff,low?3.2:4.4);key.position.set(-3,5,5);scene.add(key);const fill=new THREE.DirectionalLight(0x8fa7d8,1.65);fill.position.set(3,1,4);scene.add(fill);const rim=new THREE.PointLight(cfg.accent,low?7:12,8);rim.position.set(2,1,-2);scene.add(rim)
   let dragging=false,lastX=0,lastY=0,lastPinch=0,targetY=.18,currentY=.18,targetX=0,currentX=0,targetZoom=8.35;const pointers=new Map()

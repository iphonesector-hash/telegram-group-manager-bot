...
const TABS=[['ai','◈','دستیار'],['weather','◉','هوا'],['translate','文','ترجمه'],['calc','∑','حسابگر'],['convert','⇄','تبدیل'],['password','⌁','رمزساز'],['random','⚄','تصادفی']]
const UNITS={length:{label:'طول',units:{متر:1,کیلومتر:1000,سانتی‌متر:.01,میلی‌متر:.001,مایل:1609.344,فوت:.3048,اینچ:.0254}},weight:{label:'وزن',units:{کیلوگرم:1,گرم:.001,میلی‌گرم:.000001,پوند:.45359237,اونس:.0283495}},temperature:{label:'دما',units:{سلسیوس:1,فارنهایت:1,کلوین:1}},volume:{label:'حجم',units:{لیتر:1,میلی‌لیتر:.001,گالن:3.78541,پیمانه:.236588}}}
...
export default function ToolsPage(){
  ...
  function convert(){var n=Number(value);if(!Number.isFinite(n))return setOutput('مقدار معتبر وارد کن.');if(unitType==='temperature'){var c=from==='سلسیوس'?n:from==='فارنهایت'?(n-32)*5/9:n-273.15;var result=to==='سلسیوس'?c:to==='فارنهایت'?c*9/5+32:c+273.15;return setOutput(`${n.toLocaleString('fa-IR')} ${from} = ${result.toLocaleString('fa-IR',{maximumFractionDigits:2})} ${to}`)}var units=UNITS[unitType].units;setOutput(`${n.toLocaleString('fa-IR')} ${from} = ${(n*units[from]/units[to]).toLocaleString('fa-IR',{maximumFractionDigits:6})} ${to}`)}
  function generatePassword(){var length=Math.max(8,Math.min(64,Number(value)||18)),chars='ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#$%&*_-',bytes=new Uint32Array(length);crypto.getRandomValues(bytes);setOutput(Array.from(bytes,n=>chars[n%chars.length]).join(''))}
  function randomize(){var parts=String(input||'').split(/[،,\n]+/).map(x=>x.trim()).filter(Boolean);if(parts.length)setOutput('انتخاب تصادفی: '+parts[Math.floor(Math.random()*parts.length)]);else{var max=Math.max(2,Number(value)||100);setOutput('عدد تصادفی: '+(1+Math.floor(Math.random()*max)).toLocaleString('fa-IR'))}}
  ...
  return <div className="page fade-up">
    ...
    {!['convert','password','random'].includes(tab)&&<div className="glass" style={{padding:15}}>
    ...
    {tab==='password'&&<div className="glass tool-special"><small>SECURE GENERATOR</small><h3>رمزساز امن آفلاین</h3><p>رمز داخل گوشی و با مولد رمزنگاری مرورگر ساخته می‌شود و به سرور ارسال نمی‌شود.</p><input inputMode="numeric" value={value} onChange={e=>setValue(e.target.value)} placeholder="طول رمز؛ مثلاً ۱۸"/><button className="btn btn-primary" onClick={generatePassword}>ساخت رمز امن</button></div>}
    {tab==='random'&&<div className="glass tool-special"><small>RANDOM CORE</small><h3>انتخاب‌گر تصادفی</h3><p>گزینه‌ها را با ویرگول یا خط جدید جدا کن؛ اگر خالی باشد یک عدد می‌سازد.</p><textarea value={input} onChange={e=>setInput(e.target.value)} rows="5" placeholder={'فیلم، بازی، پیاده‌روی\nیا هر انتخاب دیگر'}/><input inputMode="numeric" value={value} onChange={e=>setValue(e.target.value)} placeholder="حداکثر عدد؛ پیش‌فرض ۱۰۰"/><button className="btn btn-primary" onClick={randomize}>انتخاب تصادفی</button></div>}

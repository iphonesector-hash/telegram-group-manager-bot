...
export default function WalletPage() {
  ...
  return (
    <div className="page fade-up">
      ...
      {tab==='history' && <div className="glass" style={{overflow:'hidden'}}>{loadingTx && <div style={{padding:20,textAlign:'center',color:'var(--muted)'}}>در حال دریافت تراکنش‌ها...</div>}{!loadingTx && transactions.length===0 && <div style={{padding:20,textAlign:'center',color:'var(--muted)'}}>هنوز تراکنشی ثبت نشده.</div>}{!loadingTx && transactions.map(function(tx,i){var earn=Number(tx.amount)>=0;return <div key={tx.id||i} style={{display:'flex',alignItems:'center',gap:12,padding:'13px 16px',borderBottom:i<transactions.length-1?'1px solid var(--border)':'none'}}><div style={{width:38,height:38,borderRadius:'50%',background:earn?'rgba(34,216,122,.12)':'rgba(255,79,106,.12)',display:'flex',alignItems:'center',justifyContent:'center'}}>{earn?'⬆️':'⬇️'}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{tx.label}</div><div style={{fontSize:11,color:'var(--muted)'}}>{formatDate(tx.date)}</div></div><div style={{fontWeight:700,color:earn?'var(--green)':'var(--red)'}}>{earn?'+':''}{Number(tx.amount||0).toLocaleString()} 🪙</div></div>})}</div>}
    </div>
  )
}

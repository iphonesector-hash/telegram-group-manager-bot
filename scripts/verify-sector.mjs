...
for (const item of ['scrap_cap','engineer_cap','commander_cap','captain_hat','elite_crown','halo_core','welder_mask','round_goggles','mono_visor','combat_visor','patched_vest','utility_jacket','officer_coat','neon_armor','royal_chassis','singularity_core','blue_shell','gold_shell','tool_pack','jetpack','mini_cape','neon_wings','ion_wings','wrench','data_pad','game_pad','plasma_tool','pulse_aura','quantum_aura','star_aura']) expect(kit3d.includes(item), `missing unique 3D equipment: ${item}`)
for (const room of ['workshop_bg','neon_city_bg','orbit_bg','command_room_bg']) expect(kit3d.includes(room), `missing 3D room: ${room}`)
for (const action of ['feed','clean','sleep','repair','charge','play','train','learn','evolution']) expect(kit3d.includes(`action==='${action}'`), `missing 3D action animation: ${action}`)

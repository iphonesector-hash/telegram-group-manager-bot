...
const nativeArcade = read('src/components/games/SectorNativeArcade.jsx')
...
for(const game of ['core2048','sector_snake','sector_memory'])expect(gamesPage.includes(game)&&nativeArcade.includes(game),`missing verified native game: ${game}`)
expect(gamesPage.includes('submitGameScore'), 'native arcade scores must sync through the verified score API')
for(const host of ['hextris.github.io','tvanas.nl','chandrakant-mane.github.io'])expect(!gamesPage.includes(host),`unreliable external arcade host remains: ${host}`)
